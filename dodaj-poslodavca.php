...
if(isset($_POST)) {
	...
	$sql = "SELECT email FROM poslodavac WHERE email='$email'";
	...
	if($result->num_rows == 0) {
		...
		if($uploadOk == false) {
			header("Location: registruj-poslodavca.php");
			exit();
		}

		//sql new registration insert query
		$sql = "INSERT INTO poslodavac(name, imefirme, grad, kontakt, website, email, password, biografija, logo) VALUES ('$name', '$imefirme', '$grad', '$kontakt', '$website', '$email', '$password', '$biografija', '$file')";

		if($conn->query($sql)===TRUE) {

			//If data inserted successfully then Set some session variables for easy reference and redirect to poslodavac login
